...
| 현재 상태 | **Phase 1 완료** ✅ (2026-06-08) **Phase 2 완료** ✅ (2026-06-10) Notion API 통합 + 어드민 UI 완성. Phase 3 진행 예정 |
...
- [ ] `components/quote/quote-display.tsx` 생성 (F006)
...
- [ ] `components/quote/pdf-download-button.tsx` 생성 (F007)
  - `"use client"` 선언
...
- [ ] `app/quote/[id]/` 디렉토리 생성 (F006, F007)
...
- [ ] Notion API 호출 실패 시 에러 토스트 표시 (F011)
...
- [ ] `app/quote/[id]/error.tsx` 추가 (Notion API 오류용 Error Boundary)
...
- [ ] `/quote/[유효한-quoteId]` 접속 시 견적서 세부 정보 정상 표시
- [ ] `/quote/[존재하지-않는-id]` 접속 시 404 페이지 표시
- [ ] "PDF 다운로드" 버튼 클릭 시 견적서 내용이 포함된 PDF 파일 브라우저 다운로드
- [ ] 공개 URL 접속 시 사이드바/네비바 미표시 (독립 레이아웃)
- [ ] Notion API 오류 발생 시 에러 메시지 표시 (빈 화면 방지)
...
| Phase | 기간 | 핵심 산출물 | 검증 방법 |
|-------|------|-------------|-----------|
| Phase 1 | 3일 | Notion API 연동 기반 (`lib/notion.ts`, `notion-helpers.ts`, Server Actions) | `getQuotes()` 호출 성공, DB 데이터 반환 확인 |
| Phase 2 | 5일 | 어드민 견적서 CRUD UI (`/quotes`, `/quotes/new`) | 전체 CRUD 동작, 공유 링크 복사 |
| Phase 3 | 4일 | 공개 견적서 뷰 + PDF (`/quote/[id]`) | 공개 URL 접근, PDF 다운로드 |
| Phase 4 | 2일 | Vercel 프로덕션 배포 | E2E 전체 여정 검증 |
| **합계** | **14일** | **MVP 완성** | **프로덕션 E2E 검증** |
